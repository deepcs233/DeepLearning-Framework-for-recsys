...
TEST_DATA_PATH = 'data/round1_ijcai_18_test_b_20180418_t.txt'

TRAIN_DATA_PATH = 'data/round1_ijcai_18_train_20180301_t.txt'
import time
...
def zuhe(data):
    ...
    for col in ['item_sales_level', 'item_price_level', 'item_collected_level',  'sale_price','sale_collect', 'price_collect',
                'user_gender_id', 'user_age_level', 'user_occupation_id', 'user_star_level','gender_age','gender_star',
...
def item(data):
    ...
    for col in ['item_city_id', 'item_sales_level', 'item_collected_level', 'item_pv_level']:
...
def user_item(data):
    ...
    # item_id
    for col in ['item_brand_id','item_city_id','item_price_level',
...
def lgbCV(train, test):
    col = [c for c in train if
           c not in ['is_trade', 'item_category_list', 'item_property_list', 'predict_category_property', 'instance_id',
                     'context_id', 'realtime', 'context_timestamp', 'user_gender_id']]
    X = train[col]
    ...
    max_depth_d = [0.01]
    res_dict = {}
    for n_leaves in num_leaves_d:
        for m_depth in max_depth_d:
            lgb0 = lgb.LGBMClassifier(
                objective='binary',
                # metric='binary_error',
                num_leaves=32,
                max_depth=7,
                learning_rate=m_depth,
                seed=2018,
                colsample_bytree=0.8,
                # min_child_samples=8,
                subsample=0.9,
                n_estimators=n_leaves)
            ...
            print(str(n_leaves) + ',' + str(m_depth))
            print('误差 ', log_loss(test['is_trade'], test['pred']))
            res_dict[str(n_leaves) + ',' + str(m_depth)] = (log_loss(test['is_trade'], test['pred']), best_iter)
    best_score = 1.0
    for n_leaves in num_leaves_d:
        for m_depth in max_depth_d:
            print(str(n_leaves) + ',' + str(m_depth) + ' :' + str(res_dict[str(n_leaves) + ',' + str(m_depth)][0]))
            if res_dict[str(n_leaves) + ',' + str(m_depth)][0] < best_score:
                best_score = res_dict[str(n_leaves) + ',' + str(m_depth)][0]
                best_iter = res_dict[str(n_leaves) + ',' + str(m_depth)][1]
    return best_iter
...
def sub(train, test, best_iter):
    ...
    sub=pd.read_csv(TEST_DATA_PATH, sep="\s+")
    sub=pd.merge(sub,sub1,on=['instance_id'],how='left')
    sub=sub.fillna(0)
    #sub[['instance_id', 'predicted_score']].to_csv('result/result0320.csv',index=None,sep=' ')
    sub[['instance_id', 'predicted_score']].to_csv('res/result0326.txt',sep=" ",index=False)


if __name__ == "__main__":
    train = pd.read_csv(TRAIN_DATA_PATH, sep="\s+")
    test = pd.read_csv(TEST_DATA_PATH, sep="\s+")
    ...
    del data['si_a']
    del data['si_b']
    for col in ['user_age_level_user_prob', 'item_id_shop_cnt', 'user_occupation_id_user_prob', 'user_gender_id_user_prob', 'item_pv_level', 'item_sales_level_price_cnt', 'item_pv_level_price_cnt', 'item_id_shop_rev_cnt', 'normal_shop', 'ui_a', 'item_collected_level_price_cnt', 'item_collected_level_user_gender_prob', 'shop_star_level_user_occ_cnt', 'item_pv_level_user_gender_prob', 'item_city_id_item_prob', 'shop_star_level', 'item_city_id', 'user_occupation_id_user_gender_prob', 'item_price_level_user_occ_prob']:
        del data[col]
    "----------------------------------------------------线下----------------------------------------"
